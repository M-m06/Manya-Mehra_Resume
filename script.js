...
photoTiles.forEach((tile) => {
  tile.addEventListener("click", () => {
    ...
    if (featuredPhoto) {
      featuredPhoto.src = tile.dataset.image;
      featuredPhoto.alt = tile.dataset.title || "Portfolio gallery photo";
    }

    if (featuredKicker) featuredKicker.textContent = tile.dataset.kicker;
    if (featuredTitle) featuredTitle.textContent = tile.dataset.title;
    if (featuredDescription) featuredDescription.textContent = tile.dataset.description;
  });
});
